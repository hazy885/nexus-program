...
- `senior-review`, `security-review`, `fact-check`, `circuit-breaker`, `prompt-coach` → subagents (the independent passes: adversarial review + threat-led security review + evidence verification + stop-the-line on dead-end/no-value work + a retro coach that teaches the human better prompting).
